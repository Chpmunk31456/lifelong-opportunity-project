...
from document_safety import (  # noqa: E402
    parse_xml_part,
...
def calc_dpi_via_ooxml_docx(input_path: str, max_w_px: int, max_h_px: int) -> int:
    ...
    root = parse_xml_part(xml, "word/document.xml")
...
def _build_lo_env(user_profile: str) -> dict:
    ...
    env["XDG_CONFIG_HOME"] = join(user_profile, "xdg_config")
    env["XDG_CACHE_HOME"] = join(user_profile, "xdg_cache")
...
def _run_cmd(cmd: list[str], env: dict, verbose: bool) -> subprocess.CompletedProcess:
    ...
    A subprocess timeout is intentionally deferred until representative
    LibreOffice runtimes have been measured. OS-level process isolation is also
    deferred to a later sandboxing phase.
    """
    ...
    if verbose:
        print("[render_docx] running " + basename(cmd[0]) + " with isolated temporary paths")
        if proc.stdout:
            print(proc.stdout)
        if proc.stderr:
            print(proc.stderr)
...
def convert_to_pdf(
    ...
    def _log_result(label: str, cmd: list[str], proc: subprocess.CompletedProcess) -> None:
        logs.append(f"--- {label} ---")
        logs.append("COMMAND: " + basename(cmd[0]) + " (paths redacted)")
    ...
    cmd_pdf = [
        r"C:\Users\alber\OneDrive\Documents\Clauade Code\LibreOffice_26.2.5_Win_x86-64\SourceDir\LibreOffice\program\soffice.com",
...
def main() -> None:
    ...
    out_dir = str(
        resolve_repository_path(
            args.output_dir if args.output_dir else splitext(input_path)[0],
            REPOSITORY_ROOT,
            must_exist=False,
