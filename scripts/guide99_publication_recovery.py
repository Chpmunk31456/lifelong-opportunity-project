#!/usr/bin/env python3
"""Fail-closed controlled publication carrier for Guide 99."""
...
QA = ROOT / "qa"
ENGLISH_BLOB = "c16332f361206af0b137d6ff8eda9300a445c9b0"
PAIRS = [
...
def preflight() -> tuple[list[Path], list[str]]:
    ...
    for p, t in zip(files, texts):
        for value in ("19-4013.00", "22100", "31160"):
            if value not in t:
                problems.append(f"{p.name}: missing controlled occupation value {value}")
    ...
    for url in urls:
        proc = subprocess.run(
            ["curl", "-L", "-sS", "-o", "/dev/null", "-w", "%{http_code}",
             "--connect-timeout", "15", "--max-time", "35", "-A", "Mozilla/5.0 Guide99-QA", url],
...
def build_documents() -> None:
    lua = Path("/tmp/guide99_url_safe.lua")
    lua.write_text(
    ...
    for stem in ("GUIDE_99_ENGLISH_v2", "GUIDE_99_SPANISH_es-419_v2", "GUIDE_99_PORTUGUESE_pt-BR_v2"):
        md = OUT / f"{stem}.md"
...
def write_evidence(url_count: int, render: dict) -> None:
    editions = []
    for lang, stem in (
        ("en", "GUIDE_99_ENGLISH_v2"),
        ("es-419", "GUIDE_99_SPANISH_es-419_v2"),
        ("pt-BR", "GUIDE_99_PORTUGUESE_pt-BR_v2"),
    ):
        docx = OUT / f"{stem}.docx"
        pdf = OUT / f"{stem}.pdf"
    ...
    manifest = {
        "guide": "99", "occupation": "Food Science Technician", "build_date": "2026-08-22",
        "status": "PASS", "editions": editions, "reader_verification_urls": url_count,
        "english_source_blob": ENGLISH_BLOB,
        "assurance_boundary": "Internal controlled publication QA; not independent certification, certified translation, food-safety/regulatory/legal/engineering/occupational-safety/cybersecurity/privacy/accessibility certification, funding approval, employment guarantee, or earnings guarantee.",
    }
...
def build() -> None:
    ...
    print(f"Guide 99 controlled publication build PASS ({len(urls)} shared URLs)")
...
def close_status() -> None:
    ...
    assert not d.get("blockers")
    for required in (
        QA / "GUIDE_99_PUBLICATION_QA_09.md",
        QA / "GUIDE_99_RELEASE_AUDIT_10.md",
        OUT / "GUIDE_99_PUBLICATION_QA_MANIFEST.json",
        OUT / "SHA256SUMS.txt",
        OUT / "RENDER_QA.json",
    ):
    ...
    d["stages"]["publication"] = {"status": "PASS", "evidence": [
        ...
        "project/revision-2026/guide-99/publication-candidate/GUIDE_99_PUBLICATION_QA_MANIFEST.json",
        "project/revision-2026/guide-99/publication-candidate/SHA256SUMS.txt",
        "project/revision-2026/guide-99/publication-candidate/RENDER_QA.json",
    ]}
    ...
    print("Guide 99 publication and release status closed PASS")


def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in {"build", "close-status"}:
        raise SystemExit("usage: guide99_publication_recovery.py {build|close-status}")
