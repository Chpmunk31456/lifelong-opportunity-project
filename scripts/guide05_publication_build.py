#!/usr/bin/env python3
"""Build and QA Guide 05 trilingual publication candidates.
...
legal/tax review, or publication approval.
"""
...
SOURCE_DIR = ROOT / "project/revision-2026/guide-05/source"
OUT = ROOT / "project/revision-2026/guide-05/publication-candidate"
PDF_DIR = OUT / "pdf"
RENDER_DIR = OUT / "rendered"
EXPECTED_SHARED_URLS = 20

EDITIONS = {
    "English": (
        SOURCE_DIR / "GUIDE_05_ENGLISH_WORKING_MASTER_v2.md",
        "LIFELONG OPPORTUNITY GUIDE 05",
        "en-US",
    ),
    "es-419": (
        SOURCE_DIR / "GUIDE_05_SPANISH_LATAM_WORKING_MASTER_v2.md",
        "GUÍA DE OPORTUNIDADES PARA TODA LA VIDA 05",
        "es-419",
    ),
    "pt-BR": (
        SOURCE_DIR / "GUIDE_05_PORTUGUESE_BR_WORKING_MASTER_v2.md",
        "GUIA DE OPORTUNIDADES PARA TODA A VIDA 05",
        "pt-BR",
    ),
}
...
def main() -> int:
    ...
    for locale, (source, title, lang_code) in EDITIONS.items():
        ...
        for anchor in ("BLS", "NOC 12200", "NOC 13102", "SENA", "Servicio Público de Empleo"):
    ...
    for locale, (source, _title, _lang_code, _text, _urls) in sources.items():
        docx = OUT / f"Lifelong_Opportunity_Guide_05_Bookkeeping_and_Payroll_Specialist_{locale}_v2.0.docx"
    ...
    subprocess.run([
        ...
        *[str(OUT / f"Lifelong_Opportunity_Guide_05_Bookkeeping_and_Payroll_Specialist_{locale}_v2.0.docx") for locale in EDITIONS],
    ], check=True)

    report = {
        "guide": "05",
        "occupation": "Bookkeeping and Payroll Specialist",
        "version": "2.0",
        ...
        "legal_or_tax_review_certification": False,
    ...
    for locale, (_source, title, lang_code, _text, source_urls) in sources.items():
        docx = OUT / f"Lifelong_Opportunity_Guide_05_Bookkeeping_and_Payroll_Specialist_{locale}_v2.0.docx"
        pdf = PDF_DIR / f"Lifelong_Opportunity_Guide_05_Bookkeeping_and_Payroll_Specialist_{locale}_v2.0.pdf"
    ...
    manifest = OUT / "GUIDE_05_PUBLICATION_QA_MANIFEST.json"
    manifest.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    checksums = OUT / "SHA256SUMS.txt"
    candidate_base = Path("project/revision-2026/guide-05/publication-candidate")
    checksums.write_text("\n".join(
        f"{item['sha256']}  {Path(item['path']).relative_to(candidate_base)}"
        for item in report["files"]
    ) + "\n", encoding="utf-8")

    print("Guide 05 trilingual publication candidate build and automated QA: PASS")
